        ...
        switch ($('select#choice').val()) {
            case "1":
                ...
                $('#question').append($('<div></div>').append(
                    ...
                    $('<input type="text" name="'+$.trim(data.name)+'">')));
                add_to_input();
            ...
            case "2":
                ...
                $('#question').append($('<div></div>').append(
                    $('<label>'+$.trim(data.question)+'</label>'),
                    $('<textarea type="text" name="'+$.trim(data.name)+'"></textarea>')));
                add_to_input();
                break;
            case "3":
                var obj = Object(null);
                ...
                $.each(data.name, function(index, val) {
                    mydiv.append($('<div><span>'+$.trim(val)+'</span><input type="checkbox" name="'+$.trim(val)+'"></div>'));
                });
            ...
            case "4":
                ...
                mydiv.append($('<label>'+$.trim(data.question)+'</label>'));
                $.each(data.name, function(index, val) {
                    mydiv.append($('<div><input type="radio" name="'+$.trim(data.question)+'"><span>'+$.trim(val)+'</span></div>'));
                });
                $('#question').append(mydiv);
                add_to_input();
                break;
            case "5":
                ...
                mydiv = $('<div></div>');
                mydiv.append($('<label>'+$.trim(data.question)+'</label>'));
                myselect = $('<select name="'+$.trim(data.question)+'"></select>');
